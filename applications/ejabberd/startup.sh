...
ctr_image='ghcr.io/processone/ejabberd:master'
restart_policy='unless-stopped'
## container specific variables
ctr_uid='9000'
app_data="$docker_dir/ejabberd/appdata/ejabberd"
listen_port='5222 5223 5259 5270 5280 5443'
ctl_on_create='register admin example.net Test12345678! ;
				module_install mod_cron'
...
podman run -d --pod="$pod_name" \
	...
	-v $app_data/conf/ejabberd.yml:/opt/ejabberd/conf/ejabberd.yml \
	-v $app_data/dhparam/dh.pem:/opt/ejabberd/dhparam/dh.pem \
	-v $app_data/database:/opt/ejabberd/database \
	-v $app_data/tls:/opt/ejabberd/tls \
	-e CTL_ON_CREATE=" $ctl_on_create " \
	-e CTL_ON_START=" $ctl_on_start " \
	"$ctr_image"

# create xmpp-http-upload container
## general variables
app_name='http-upload'
ctr_name="$pod_name-$app_name"
ctr_image='localhost/nginx-http-upload:latest'
restart_policy='unless-stopped'
## container specific variables
ctr_uid='101'
app_data="$docker_dir/ejabberd/appdata/nginx"
listen_port='8888'

info 'Adjust host directory/file permissions ...'
podman unshare chown -R $ctr_uid:$ctr_uid $app_data

info "Build container image for $ctr_name ..."
podman build -t $ctr_image -f $docker_dir/ejabberd/image-nginx/Dockerfile \
	$docker_dir/ejabberd/image-nginx/.
...
podman run -d --pod="$pod_name" \
	...
	--cap-drop=ALL \
	--security-opt no-new-privileges \
	-v $app_data/nginx.conf:/etc/nginx/nginx.conf:ro \
	-v $app_data/conf.d/default.conf:/etc/nginx/conf.d/default.conf:ro \
	-v $app_data/upload:/http-upload \
