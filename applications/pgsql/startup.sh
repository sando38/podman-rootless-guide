...
ctr_image='docker.io/postgres:15-alpine'
restart_policy='unless-stopped'
## container specific variables
ctr_uid='70'
app_data="$docker_dir/pgsql/appdata/data"
secrets='postgres_initdb_args postgres_password postgres_user postgres_db'
listen_port='5432'
...
podman run -d --pod="$pod_name" \
	...
	--shm-size=256MB \
	-v $app_data/data:/var/lib/postgresql/data:rw \
	-e POSTGRES_INITDB_ARGS_FILE=/run/secrets/postgres_initdb_args \
	-e POSTGRES_PASSWORD_FILE=/run/secrets/postgres_password \
	-e POSTGRES_USER_FILE=/run/secrets/postgres_user \
	-e POSTGRES_DB_FILE=/run/secrets/postgres_db \
	--secret postgres_initdb_args \
